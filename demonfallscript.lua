...
TeleportsTab:CreateDropdown({
    Name = "Jogadores no Servidor ("..#playerTeleportLocations..")", Options = playerNames, CurrentOption = {playerNames[1]},
    MultipleOptions = false, Flag = "PlayerTeleportDropdown",
    Callback = function(S) 
        local name=S[1]; 
        if name=="-- Selecione um Jogador --" or name=="Nenhum outro jogador encontrado" then return end; 
        local target=playerTeleportLocations[name]; 
        if target and target.Character and target.Character:FindFirstChild("HumanoidRootPart") then 
            local char=LocalPlayer.Character; 
            local hrp=char and char:FindFirstChild("HumanoidRootPart"); 
            if hrp then 
                hrp.CFrame=target.Character.HumanoidRootPart.CFrame*CFrame.new(0,5,0); 
                Rayfield:Notify({Title="Teleporte",Content="Seguindo: "..name,Duration=3}) 
            end 
        else 
            Rayfield:Notify({Title="Erro",Content="Não foi possível encontrar o personagem de "..name,Duration=4}) 
        end 
    end, 
})
...
PlayerTab:CreateSection("Super Resistência")
PlayerTab:CreateSlider({ Name = "Resistência a Dano (%)", Min = 0, Max = 90, Default = 50, Callback = function(Value) resistancePercent = Value / 100 end })
PlayerTab:CreateToggle({ Name = "Ativar Super Resistência", CurrentValue = false, Flag = "DamageReductionToggle", Callback = function(Value) damageReductionEnabled = Value; if Value then if LocalPlayer.Character then applyDamageReduction(LocalPlayer.Character) end; drConnection.CharacterAdded = LocalPlayer.CharacterAdded:Connect(applyDamageReduction) else removeDamageReduction() end end })
PlayerTab:CreateSection("Modificações de Movimento")
PlayerTab:CreateSlider({
    Name = "WalkSpeed", Min = 16, Max = 200, Default = 16, Suffix = " studs/s",
    Flag = "WalkSpeedSlider_EzyHub",
    Callback = function(Value)
        print("WalkSpeed alterado para:", Value)
