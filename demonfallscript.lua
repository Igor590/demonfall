...
for i, descendant in pairs(Workspace:GetDescendants()) do
    if descendant:IsA("Humanoid") then
        local model = descendant.Parent; local player = Players:GetPlayerFromCharacter(model); local parentName = model.Parent.Name;
        if not player and (parentName == "Npcs" or parentName == "Shops") then
            local rootPart = model:FindFirstChild("HumanoidRootPart");
            if rootPart then
                npcCount = npcCount + 1; local uniqueName = model.Name .. " (" .. parentName .. ")";
                if foundNpcs[uniqueName] then uniqueName = uniqueName .. " #" .. npcCount end
                foundNpcs[uniqueName] = rootPart.Position; table.insert(foundNpcNames, uniqueName)
            end
        end
    end
end
...
local playerTeleportLocations, playerNames = {}, {};
for _, player in pairs(Players:GetPlayers()) do
    if player ~= LocalPlayer then table.insert(playerNames, player.Name); playerTeleportLocations[player.Name] = player end
end
if #playerNames > 0 then table.insert(playerNames, 1, "-- Selecione um Jogador --") else playerNames = {"Nenhum outro jogador encontrado"} end
print("--- [ PRÉ-SCAN DE JOGADORES FINALIZADO: " .. #playerTeleportLocations .. " JOGADORES ENCONTRADOS ] ---")

-- LÓGICA DO AUTO-FARM "QUE APRENDE"
local farmingEnabled = false; local delayAfterCollect = 0.5; local delayAfterCycle = 60;
local function startFarming()
    ...
    while farmingEnabled do
        for i, spawnPos in pairs(trinketSpawnPoints) do
            ...
            humanoidRootPart.CFrame = CFrame.new(spawnPos) * CFrame.new(0, 3, 0); task.wait(1);
            ...
            for _, descendant in pairs(Workspace:GetDescendants()) do
                ...
                if descendant:IsA("ProximityPrompt") then
                    ...
                    if itemPart and itemPart:IsA("BasePart") and (itemPart.Position - spawnPos).Magnitude < 30 and not itemModel:FindFirstChildOfClass("Humanoid") then
                        print(">> Trinket '"..itemModel.Name.."' encontrada. Coletando...")
                        ...
                        task.wait(delayAfterCollect); trinketFound = true; break
                    end
                end
                if descendant.Name == "Spawn" then
                    local parentModel = descendant:FindFirstAncestorOfClass("Model")
                    if parentModel and not parentModel:FindFirstChildOfClass("Humanoid") then
                        local position = nil
                        if descendant:IsA("BasePart") then position = descendant.Position elseif descendant:IsA("ObjectValue") and descendant.Value and descendant.Value:IsA("BasePart") then position = descendant.Value.Position end
                        if position and not spawnsConhecidos[position] then
                            spawnsConhecidos[position] = true; table.insert(trinketSpawnPoints, position)
                            print("+++ NOVO PONTO DE SPAWN DESCOBERTO! Total: " .. #trinketSpawnPoints .. " +++")
                        end
                    end
                end
            end
            if not trinketFound then print("Nenhuma trinket no ponto #"..i) end
        end
        print("Ciclo completo. Aguardando " .. delayAfterCycle .. "s...")
...
-- LÓGICA ESP
local espEnabled = false; local activeHighlights = {};
local function updateEsp()
    local healthBarSize = UDim2.new(0, 125, 0, 18); local healthBarOffset = Vector3.new(0, 2.5, 0); local healthBarBackgroundColor = Color3.fromRGB(20, 20, 20); local playerHealthColor = Color3.fromRGB(80, 80, 255); local mobHealthColor = Color3.fromRGB(255, 50, 50); local textColor = Color3.fromRGB(255, 255, 255);
    while espEnabled do
        ...
        local function createHealthBar(character, healthColor)
            ...
            local gui = Instance.new("BillboardGui"); gui.Adornee = character:FindFirstChild("Head") or character:FindFirstChild("HumanoidRootPart"); gui.Size = healthBarSize; gui.StudsOffset = healthBarOffset; gui.AlwaysOnTop = true;
            local bg = Instance.new("Frame", gui); bg.Name = "Background"; bg.Size = UDim2.new(1, 0, 1, 0); bg.BackgroundColor3 = healthBarBackgroundColor; bg.BorderSizePixel = 0;
            local bar = Instance.new("Frame", bg); bar.Name = "HealthBar"; bar.Size = UDim2.new(humanoid.Health / humanoid.MaxHealth, 0, 1, 0); bar.BackgroundColor3 = healthColor; bar.BorderSizePixel = 0;
            local text = Instance.new("TextLabel", bg); text.Name = "HealthText"; text.Size = UDim2.new(1, 0, 1, 0); text.Text = math.floor(humanoid.Health) .. "/" .. math.floor(humanoid.MaxHealth); text.TextColor3 = textColor; text.BackgroundTransparency = 1; text.Font = Enum.Font.SourceSansBold; text.TextScaled = true;
        ...
        for _, data in pairs(targets) do
            local character = data.Target; table.insert(charactersInView, character);
        ...
        for character, data in pairs(activeHighlights) do
            ...
            if humanoid and data.HealthGui then
                local bar = data.HealthGui:FindFirstChild("HealthBar", true); local text = data.HealthGui:FindFirstChild("HealthText", true);
                if bar and text then
                    local healthPercent = humanoid.Health / humanoid.MaxHealth; bar.Size = UDim2.new(healthPercent, 0, 1, 0); text.Text = math.floor(humanoid.Health) .. "/" .. math.floor(humanoid.MaxHealth)
                end
            end
        end;
        for character, data in pairs(activeHighlights) do
            if not table.find(charactersInView, character) or not character.Parent or character:FindFirstChildOfClass("Humanoid").Health <= 0 then
                if data.Highlight then data.Highlight:Destroy() end; if data.HealthGui then data.HealthGui:Destroy() end; activeHighlights[character] = nil
            end
        end;
        task.wait(0.25)
    end;
    for _, data in pairs(activeHighlights) do if data.Highlight then data.Highlight:Destroy() end; if data.HealthGui then data.HealthGui:Destroy() end end;
    activeHighlights = {}
end
